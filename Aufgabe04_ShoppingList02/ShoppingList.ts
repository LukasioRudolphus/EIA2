namespace shoppingList {
    ...
    function addValueAmount(amountEntry: any, i: number) {
        let valueAmount = document.getElementById("itemAmount" + i) as HTMLInputElement;
        valueAmount.value = amountEntry;
    }

    function addCheckmark(checkEntry: any, i: number) {
        let addMark = document.getElementById("wasBought" + i) as HTMLInputElement;
        addMark.checked = checkEntry;
    }

    function addDate(dateEntry: any, i: number) {
        let addLastDate = document.getElementById("lastPurchase" + i) as HTMLInputElement;
    ...
    function deleteEntryButton(): void {
    ...
    function makeNewEntry(this: any): void {
        deleteEntryButton();

        // umliegende Box
        let inputNewEntry: HTMLElement = document.createElement("fieldset");
    ...
    function makeEntry(): void {
        let dataNewInput = document.getElementsByClassName("inputData") as HTMLCollectionOf<HTMLInputElement>;
        let nameField: any = dataNewInput[0];
        let amountField: any = dataNewInput[1];
        let commentField: any = dataNewInput[2];
        ...
        if (nameField.value != "" && amountField.value != "") {
            ...
            let arrayNewInput: { Name: string, Amount: number, Comment: string, Bought: boolean, LastBought: string; } = {
